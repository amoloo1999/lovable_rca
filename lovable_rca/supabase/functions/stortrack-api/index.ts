...
// Safe debug: log presence (but never the values) so we can verify secrets are available
console.log('StorTrack env presence:', {
  STORTRACK_BASEURL: STORTRACK_BASEURL ? 'present' : 'missing',
  STORTRACK_USERNAME: STORTRACK_USERNAME ? 'present' : 'missing',
  STORTRACK_PASSWORD: STORTRACK_PASSWORD ? 'present' : 'missing',
});
