...
export const WIZARD_STEPS: WizardStep[] = [
  ...
  { id: 10, name: 'Export', description: 'Generate CSV reports', completed: false },
];
...
export function useRCAWizard() {
  ...
  const selectStoresForAnalysis = useCallback((stores: Store[]) => {
    setState((prev) => {
      const selected = prev.subjectStore ? [prev.subjectStore, ...stores] : stores;
  ...
  // Try to fetch metadata from Salesforce via MCP
        ...
        setState((prev) => ({
          ...
          storeMetadata: {
            ...
            [store.storeId]: {
              ...
              squareFootage: match.squareFootage,
            },
          },
        }));
        return match;
